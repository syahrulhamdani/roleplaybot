...
class BotPipelineRunner:
    ...
    async def start(
        self,
        create_task: Callable[[BotCallbacks], Awaitable[PipelineTask]],
        handle_sigint=True,
    ):
        ...
        await asyncio.wait(
            {pipeline_task, timeout_task}, return_when=asyncio.FIRST_COMPLETED
        )
    ...
    async def _participant_join_timeout(self):
        try:
            seconds = int(
                os.getenv(
                    "MAX_PARTICIPANT_JOIN_SECONDS", DEFAULT_MAX_PARTICIPANT_JOIN_SECONDS
                )
            )
            ...
            if not self._participant_joined:
                ...
                if self._task:
                    await self._task.queue_frames(
                        [StartInterruptionFrame(), EndFrame()]
                    )
